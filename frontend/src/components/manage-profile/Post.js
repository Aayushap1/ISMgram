...
const Post = (props) => {
  ...
  const handleDeletePost = async (e) => {
    ...
    if (response.ok) {
      ...
      const updatedPosts = props.posts.filter((post) => post._id !== targetId);
      props.setPosts(updatedPosts);
      const updatedUserPosts = user.posts.filter((post) => post !== targetId);
      const updatedUser = { ...user, posts: updatedUserPosts };
  ...
  return (
    <div className="post mb-4 shadow rounded-3">
      <li className="list-group-item d-flex justify-content-between align-items-center">
        ...
        <button className="btn btn-danger rounded-pill px-3 py-1" onClick={handleDeletePost}>
          Delete
        </button>
      </li>
      {/* <li className="list-group-item d-flex justify-content-between align-items-center"> */}
